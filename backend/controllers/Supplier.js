    ...
    try {
        ...
        if (supplier) {
            if (bcrypt.compareSync(password, supplier.password)) {
                ...
                const token = jwt.sign({ id: supplier._id, email: supplier.email }, secret, {
                    expiresIn: "3h",
                });

                return res.status(200).json({ success: true, user: supplier.permissionLevel, message: "Supplier authenticated", token: token });
            }
            return res.status(406).json({ success: false, user: true, message: "Password Incorrect" });
        }
        else {
            return res.status(402).json({ success: false, user: false, message: "Supplier doesn't exist" });
        }
    }
    catch (error) {
